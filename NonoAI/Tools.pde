int findSum(int index,int lReduce, int rReduce, boolean actual) {
  ...
  if(actual)
    ...
    return temp+values[index].length-(lReduce+rReduce)-1;
}
...
void checkComplete(int index, int sum) {
  if(count(index)[1] == sum) {
    for(int i=0; i<gridSize; i++) {
      if(index < gridSize && grid.cell[i][index].state == 0) grid.setState(i, index, -1);
      else if(index > gridSize && grid.cell[index%gridSize][i].state == 0) grid.setState(index%gridSize,i,-1);
    }
...
void check() {
  for(int i=0; i<isComplete.length; i++) {
    checkComplete(i, findActualSum(i));
  }
}
